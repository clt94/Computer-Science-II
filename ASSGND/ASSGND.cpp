...
Polynomial& Polynomial::operator+(Polynomial const &poly1)
{
    Polynomial temp = *this;
    temp.numberOfTerms = this->getNumberOfTerms() + poly1.getNumberOfTerms();
    for(size_t i{1}; i <= temp.getNumberOfTerms(); ++i)
    {
        if(i <= this->getNumberOfTerms())
        {
            temp.coefficients[i] = this->coefficients[i];
            temp.exponents[i] = this->exponents[i];
        }
        else
        {
            temp.coefficients[i] = poly1.coefficients[i - this->numberOfTerms];
            temp.exponents[i] = poly1.exponents[i - this->numberOfTerms];
        }
    }
    *this = temp;
    return *this;
...
Polynomial& Polynomial::operator=(Polynomial const &poly1)
{
    for(size_t i{1}; i < maxTerms; ++i)
    {
        if(i > poly1.numberOfTerms)
        {
